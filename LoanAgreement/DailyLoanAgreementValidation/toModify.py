...
info = json.load(file)
email = info["user6"]["email"]
password = info["user6"]["password"]
otp = info["user6"]["otp"]
loanID = info["user6"]["loanID"]
loanPdf = info["user6"]["loanPdf"]
...
class TestDashRepo:
    ...
    def test_keyFactStatement(self, setup_teardown):
        ...
        docChargInt = int((loanAmountInt / 100) * 1)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        disburse = loanAmountInt - (processChargeInt + docChargFloat + stampCharge + onlineConvCharge + sgstFloat + cgstFloat + insCharg)
                        disFloat = str(round(disburse, 3))[:7]
